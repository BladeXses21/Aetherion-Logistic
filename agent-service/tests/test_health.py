"""
test_health.py — тести для GET /health endpoint agent-service.
...
async def test_health_healthy_dependency_not_overwritten_on_redis_down(client_redis_down):
    """При недоступному Redis в dependencies.redis повинно бути 'error', не 'ok'."""
    response = await client_redis_down.get("/health")
    data = response.json()
    assert data["dependencies"]["redis"] == "error"
    assert data["status"] == "unhealthy"
