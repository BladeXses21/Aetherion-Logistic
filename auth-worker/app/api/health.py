...
Примітка: перевірка LTSID ("ltsid": "valid") буде додана в Story 2.1 після реалізації Chrome login.
"""
...
async def health(request: Request) -> JSONResponse:
    ...
    Перевіряє:
        - Redis: команда PING
        (Story 2.1 додасть: "ltsid": "valid" — EXISTS в Redis + TTL > 0)

    Returns:
        HTTP 200 з {"status": "healthy", "service": "auth-worker", "dependencies": {"redis": "ok"}}
    ...
    if not all_ok:
        log.warning("health_check_degraded", service="auth-worker", dependencies=dependencies)
